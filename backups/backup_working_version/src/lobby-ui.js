    ...
    onGameUpdate: async (game) => {
      ...
      if (!game.betting_phase && game.current_round && game.cards_drawn && game.cards_drawn.length > 0) {
        ...
        for (let i = 0; i < game.cards_drawn.length; i++) {
          ...
          const newPosition = Math.max(0, Math.min(10, oldPosition + movement));
...
async function loadRoomData(gameId) {
  try {
    ...
    if (playersList) {
      // Raggruppa puntate per giocatore (user_id)
      const playerData = {};
      ...
      allBets?.forEach(bet => {
        ...
        playerData[userId].bets.push(bet);
        playerData[userId].totalSpent += bet.amount;
      });

      // Mostra giocatori
      if (Object.keys(playerData).length === 0) {
        playersList.innerHTML = '<p style="color: #ccc; text-align: center;">Nessun giocatore ha ancora puntato</p>';
      } else {
        playersList.innerHTML = Object.values(playerData).map(player => {
          // Trova l'ultima puntata (quella più recente)
          const lastBet = player.bets[player.bets.length - 1];
